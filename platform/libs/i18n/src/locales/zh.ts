const translations = {
  common: {
    save: '保存',
    cancel: '取消',
    submit: '提交',
    delete: '删除',
    edit: '编辑',
    view: '查看',
    search: '搜索',
    filter: '筛选',
    export: '导出',
    import: '导入',
    loading: '加载中...',
    noData: '暂无数据',
    confirm: '确认',
    back: '返回',
    next: '下一步',
    previous: '上一步',
    close: '关闭',
    refresh: '刷新',
    actions: '操作',
    status: '状态',
    date: '日期',
    amount: '金额',
    total: '总计',
    all: '全部',
    active: '活跃',
    inactive: '未激活',
    pending: '待处理',
    approved: '已批准',
    rejected: '已拒绝',
    poweredBy: '由 Papaya AI 驱动',
    comingSoon: '即将推出',
    comingSoonDesc:
      '此功能正在由我们的AI工程团队构建，即将上线。',
    welcome: '欢迎回来',
    selectModule: '从侧边栏选择一个模块开始使用',
    getStarted: '开始使用',
    viewAll: '查看全部',
    noResults: '未找到结果。',
    newConversation: '新对话',
    stop: '停止',
    collapse: '收起',
    expand: '展开',
    somethingWentWrong: '出现错误',
    tryAgain: '重试',
    retry: '重试',
  },

  app: {
    name: 'Oasis',
  },

  nav: {
    main: '主要',
    operations: '运营',
    intelligence: '智能',
    management: '管理',
    dashboard: '仪表盘',
    claims: '理赔',
    claimsIntake: '报案',
    claimsReview: '审核队列',
    claimsAdjudication: '理算',
    claimsHistory: '历史记录',
    policies: '保单',
    policiesBrowse: '浏览',
    policiesEndorsements: '批单',
    policiesRenewals: '续保',
    policiesServicing: '保全',
    underwriting: '核保',
    underwritingApplications: '投保申请',
    underwritingRisk: '风险评估',
    underwritingPricing: '定价',
    fwa: 'FWA检测',
    fwaAlerts: '警报',
    fwaInvestigations: '调查',
    fwaRules: '规则引擎',
    reporting: '报表',
    reportingDashboards: '仪表盘',
    reportingReports: '报告',
    reportingAnalytics: '分析',
    reportingLoss: '赔付管理',
    providers: '医疗机构',
    providersDirectory: '目录',
    providersContracts: '合同',
    providersPerformance: '绩效',
    admin: '管理',
    adminUsers: '用户与角色',
    adminSettings: '设置',
    adminAudit: '审计日志',
    systemStatus: '系统状态',
    designSystem: '设计系统',
    aiAgents: 'AI智能体',
    drone: 'Drone',
    portal: '门户',
    docs: '文档',
    ir: '投资者关系',
    irRounds: '融资轮次',
    irInvestors: '投资者',
    irAnalytics: '分析',
  },

  dashboard: {
    title: '仪表盘',
    subtitle: '保险运营概览',
    totalClaims: '总理赔数',
    pendingReview: '待审核',
    activePolicies: '有效保单',
    fwaAlerts: 'FWA警报',
    aiProcessed: '今日AI处理量',
    lossRatio: '赔付率',
    recentActivity: '近期活动',
    quickActions: '快捷操作',
    newClaim: '新建理赔',
    newPolicy: '新建保单',
    runAnalysis: '运行分析',
    claimsTrend: '理赔趋势',
    topProviders: '优质医疗机构',
    vsLastMonth: '较上月',
    vsLastWeek: '较上周',
    growth: '增长',
    accuracy: '准确率',
    activity: {
      aiAdjudicated: 'AI已理算',
      alertFlagged: '已标记警报',
      submitted: '已提交',
      renewed: '已续保',
      documentsAnalyzed: '已分析文档',
    },
  },

  claims: {
    title: '理赔管理',
    subtitle: '处理、审核和管理保险理赔',
    newClaim: '新建理赔',
    claimId: '理赔编号',
    claimant: '索赔人',
    provider: '医疗机构',
    status: '状态',
    amount: '金额',
    submittedDate: '提交日期',
    statuses: {
      submitted: '已提交',
      underReview: '审核中',
      aiProcessing: 'AI处理中',
      adjudicated: '已理算',
      approved: '已批准',
      denied: '已拒绝',
      appealed: '已申诉',
    },
    intakeTitle: '报案受理',
    intakeDesc:
      'AI驱动的理赔处理。提交、审核和理算理赔，实现智能自动化。',
    reviewTitle: '审核队列',
    reviewDesc:
      '等待人工审核的理赔。AI已标记需要人工关注。',
    adjudicationTitle: '理算',
    adjudicationDesc:
      '已审核理赔的最终理算决定。批准、拒绝或要求补充信息。',
    historyTitle: '理赔历史',
    historyDesc:
      '浏览所有已处理的理赔，包含完整的审计跟踪和决策历史。',
  },

  policies: {
    title: '保单管理',
    subtitle: '管理保单、批单和续保',
    newPolicy: '新建保单',
    policyNumber: '保单号',
    insured: '被保险人',
    product: '产品',
    effectiveDate: '生效日期',
    expiryDate: '到期日期',
    premium: '保费',
    statuses: {
      active: '有效',
      expired: '已过期',
      cancelled: '已退保',
      pending: '待生效',
    },
    browseTitle: '保单浏览器',
    browseDesc:
      '管理保险保单、批单和续保，覆盖整个投资组合。',
    endorsementsTitle: '批单',
    endorsementsDesc:
      '处理保单批单、修改和期中调整。',
    renewalsTitle: '续保',
    renewalsDesc:
      '跟踪即将到来的保单续保并管理续保流程。',
    servicingTitle: '保全服务',
    servicingDesc:
      '处理保全服务请求、退保和客户咨询。',
  },

  underwriting: {
    title: '核保',
    subtitle: '评估风险、定价保单和管理投保申请',
    newApplication: '新建申请',
    applicant: '投保人',
    riskScore: '风险评分',
    recommendedPremium: '建议保费',
    statuses: {
      pending: '待处理',
      approved: '已通过',
      declined: '已拒绝',
      referToUnderwriter: '转核保人',
      moreInfoRequired: '需补充资料',
    },
    applicationsTitle: '投保申请',
    applicationsDesc:
      'AI辅助的风险评估和定价，用于新保险申请。',
    riskTitle: '风险评估',
    riskDesc:
      'AI驱动的风险评分和分析，适用于个人申请和投资组合。',
    pricingTitle: '定价',
    pricingDesc:
      '基于风险档案的动态定价模型和保费计算。',
  },

  fwa: {
    title: 'FWA检测',
    subtitle: 'AI驱动的欺诈、浪费和滥用检测',
    alertCount: '警报数量',
    severity: {
      critical: '严重',
      high: '高',
      medium: '中',
      low: '低',
    },
    investigate: '调查',
    resolve: '处理',
    falsePositive: '误报',
    alertsTitle: 'FWA警报',
    alertsDesc:
      '由Papaya AI智能体驱动的实时欺诈、浪费和滥用检测。',
    investigationsTitle: '调查',
    investigationsDesc:
      '跟踪和管理正在进行的欺诈调查，提供完整的案件管理。',
    rulesTitle: '规则引擎',
    rulesDesc:
      '配置和管理欺诈检测规则、阈值和评分模型。',
  },

  portal: {
    title: '门户',
    subtitle: '理赔处理门户',
    dashboard: {
      title: '仪表盘',
      totalClaims: '总理赔数',
      processing: '处理中',
      awaitingApproval: '待批准',
      approved: '已批准',
      recentClaims: '近期理赔',
      newClaim: '新建理赔',
      viewAllClaims: '查看所有理赔',
    },
    claims: {
      title: '理赔',
      totalClaims: '{{count}} 条理赔',
      searchPlaceholder: '搜索理赔...',
      allStatuses: '所有状态',
      noClaims: '未找到理赔',
      noClaimsDesc: '上传文件以创建新理赔。',
      noClaimsFilterDesc: '请调整搜索条件或筛选器。',
      noClaimsEmptyDesc: '暂无理赔记录。',
      submitClaim: '提交理赔',
      claimNumber: '理赔编号',
      insuredName: '被保险人姓名',
      type: '类型',
      fwaRisk: 'FWA 风险',
      amount: '金额',
      status: '状态',
      date: '日期',
      created: '创建时间',
      pagination: '第 {{page}} 页 / 共 {{totalPages}} 页（{{total}} 条理赔）',
      statusSubmitted: '已提交',
      statusProcessing: '处理中',
      statusInReview: '审核中',
      statusApproved: '已批准',
      statusRejected: '已拒绝',
      statusPending: '待处理',
    },
    newClaim: {
      title: '新建理赔',
      claimCode: '理赔代码',
      claimCodePlaceholder: '例如 CLM-2026-001',
      uploadDocuments: '上传文件',
      dropFiles: '拖放文件到此处或点击浏览',
      acceptedFormats: 'PDF、JPG、PNG 每个不超过20MB',
      submitting: '提交中...',
      submit: '提交理赔',
    },
    claimDetail: {
      backToClaims: '理赔',
      reprocess: '重新处理',
      autoRefreshing: '处理中 — 每5秒自动刷新',
      tabs: {
        overview: '概览',
        extraction: '数据提取',
        assessment: '评估',
        medicalNecessity: '医疗必要性',
        fwa: 'FWA',
      },
    },
    overview: {
      claimInfo: '理赔信息',
      financialSummary: '财务摘要',
      insuredPerson: '被保险人',
      processTimeline: '处理时间线',
      requestedAmount: '申请金额',
      coveredAmount: '覆盖金额',
      uncoveredAmount: '未覆盖金额',
      shortfallAmount: '差额',
      diagnosis: '诊断',
      icdCode: 'ICD代码',
      admissionDate: '入院日期',
      dischargeDate: '出院日期',
      directBilling: '直接结算',
      surgery: '手术',
      certificateCode: '证书代码',
      policyNumber: '保单编号',
      corporateName: '企业',
      provider: '医疗机构',
    },
    documents: {
      title: '文件',
      noDocuments: '暂无上传文件',
      zoomIn: '放大',
      zoomOut: '缩小',
      rotate: '旋转',
    },
    agent: {
      run: '运行',
      stop: '停止',
      streaming: '流式传输中...',
      toolCalls: '工具调用',
      output: '输出',
      emptyState: '点击"运行 {{label}}"开始AI分析。',
    },
    analytics: {
      title: '分析',
      claimsOverTime: '理赔趋势',
      outcomeDistribution: '结果分布',
      topDiagnoses: '常见诊断',
      processingTimes: '处理时间',
    },
    fwaAnalytics: {
      title: 'FWA分析',
      totalFlagged: '标记总数',
      avgRiskScore: '平均风险评分',
      investigationRate: '调查率',
      confirmationRate: '确认率',
      riskDistribution: '风险分布',
      topFlags: '常见FWA标记',
      riskTrends: '风险趋势',
      topFlaggedClaims: '高风险理赔',
    },
    fwaCases: {
      title: 'FWA案例',
      flaggedQueue: '标记队列',
      activeCases: '活跃案例',
      newCase: '新建案例',
      searchCases: '搜索案例...',
      noCases: '未找到案例',
    },
    settings: {
      title: '设置',
      tenantInfo: '租户信息',
      activeModules: '活跃模块',
      assessmentConfig: '评估配置',
      medicalNecessityConfig: '医疗必要性配置',
    },
    claimStatus: {
      submitted: '已提交',
      processing: '处理中',
      in_review: '审核中',
      awaiting_approval: '待批准',
      approved: '已批准',
      rejected: '已拒绝',
      pending: '待处理',
      success: '成功',
      error: '错误',
    },
    claimType: {
      INPATIENT: '住院',
      OUTPATIENT: '门诊',
      DENTAL: '牙科',
      DAY_CASE: '日间手术',
      MATERNITY: '产科',
    },
    riskLevel: {
      LOW: '低',
      MEDIUM: '中',
      HIGH: '高',
      CRITICAL: '严重',
    },
  },

  providers: {
    title: '医疗机构管理',
    subtitle: '管理医疗服务提供商网络和合同',
    providerName: '机构名称',
    specialty: '专科',
    contractStatus: '合同状态',
    networkStatus: '网络状态',
    addProvider: '添加机构',
    directoryTitle: '医疗机构目录',
    directoryDesc:
      '管理医疗服务提供商网络、合同和绩效指标。',
    contractsTitle: '合同',
    contractsDesc:
      '查看和管理医疗机构合同、费率表和协议。',
    performanceTitle: '绩效',
    performanceDesc:
      '跟踪医疗机构绩效指标、质量评分和合规评级。',
  },

  reporting: {
    title: '报表与分析',
    subtitle: '洞察、仪表盘和赔付管理',
    generateReport: '生成报表',
    scheduledReports: '定时报表',
    lossRatio: '赔付率',
    claimsRatio: '理赔率',
    exportData: '导出数据',
    dashboardsTitle: '仪表盘',
    dashboardsDesc:
      '全面的保险分析、报表和赔付管理洞察。',
    reportsTitle: '报告',
    reportsDesc:
      '生成和安排监管、财务和运营报告。',
    analyticsTitle: '分析',
    analyticsDesc:
      '深度分析，支持自定义查询、可视化和数据探索。',
    lossTitle: '赔付管理',
    lossDesc:
      '跟踪赔付率、准备金充足性和理赔发展三角形。',
  },

  admin: {
    title: '系统管理',
    subtitle: '管理用户、角色、租户和系统设置',
    users: '用户',
    roles: '角色',
    tenants: '租户',
    systemSettings: '系统设置',
    auditLog: '审计日志',
    aiAgentConfig: 'AI智能体配置',
    addUser: '添加用户',
    inviteUser: '邀请用户',
    editUser: '编辑用户',
    createUser: '创建用户',
    table: {
      name: '姓名',
      type: '类型',
      level: '级别',
      department: '部门',
      lastLogin: '最后登录',
      createdAt: '创建时间',
      createdBy: '创建者',
      openMenu: '打开菜单',
      edit: '编辑',
      delete: '删除',
      impersonate: '模拟身份',
      allowImpersonation: '允许身份模拟',
      disallowImpersonation: '禁止身份模拟',
      grantCanImpersonate: '授予模拟权限',
      revokeCanImpersonate: '撤销模拟权限',
      revokeSessions: '撤销所有会话',
    },
    searchPlaceholder: '按姓名或邮箱搜索...',
    filters: {
      allTypes: '所有类型',
      allLevels: '所有级别',
    },
    userTypes: {
      insurer: '保险公司',
      broker: '经纪人',
      provider: '医疗机构',
      papaya: 'Papaya',
    },
    userLevels: {
      admin: '管理员',
      executive: '高管',
      manager: '经理',
      staff: '员工',
      viewer: '查看者',
    },
    lastLogin: {
      never: '从未',
      justNow: '刚刚',
      minutesAgo: '{{count}}分钟前',
      hoursAgo: '{{count}}小时前',
      daysAgo: '{{count}}天前',
    },
    emptyState: {
      noUsersFound: '未找到用户',
      adjustFilters: '请尝试调整搜索条件或筛选器。',
      addFirstUser: '开始添加第一个用户。',
    },
    pagination: {
      showing: '显示 {{start}}-{{end}} 共 {{total}} 个用户',
      pageOf: '第 {{page}} 页 / 共 {{totalPages}} 页',
    },
    deleteDialog: {
      title: '删除用户',
      description: '确定要删除 {{name}} 吗？此操作无法撤销。',
      deleting: '删除中...',
    },
    impersonateDialog: {
      title: '模拟用户身份',
      description: '您将以 {{name}} 的身份查看 Oasis。所有操作将被记录。',
      confirm: '开始模拟',
      impersonating: '正在模拟...',
    },
    revokeSessionsDialog: {
      title: '撤销所有会话',
      description: '此操作将立即强制 {{name}} 退出所有设备。用户需要重新登录。',
      confirm: '撤销会话',
      revoking: '正在撤销...',
    },
    form: {
      name: '姓名',
      namePlaceholder: '张三',
      email: '邮箱',
      emailPlaceholder: 'zhangsan@example.com',
      phone: '手机',
      phonePlaceholder: '+86 1XX XXXX XXXX',
      type: '类型',
      level: '级别',
      jobTitle: '职位',
      titlePlaceholder: '理赔副总裁',
      department: '部门',
      departmentPlaceholder: '运营部',
      tenant: '租户',
      selectTenant: '选择租户',
      allTenants: '所有租户',
    },
    dialog: {
      editDescription: '请更新以下用户信息。',
      createDescription: '请填写信息以创建新用户。',
      saving: '保存中...',
      creating: '创建中...',
      saveChanges: '保存更改',
      createUser: '创建用户',
    },
    validation: {
      nameRequired: '姓名为必填项',
      emailRequired: '邮箱为必填项',
      invalidEmail: '邮箱地址无效',
      tenantRequired: '租户为必填项',
    },
    usersTitle: '用户与角色',
    usersDesc:
      '管理用户、角色、系统设置和审查审计跟踪。',
    settingsTitle: '设置',
    settingsDesc:
      '配置系统级设置、集成和租户偏好。',
    auditTitle: '审计日志',
    auditDesc:
      '审查所有系统活动、用户操作和配置变更。',
  },

  auth: {
    signIn: '登录',
    signOut: '退出登录',
    profile: '个人资料',
    settings: '设置',
    role: '角色',
    switchOrganization: '切换组织',
    security: '安全',
    passkey: {
      title: '通行密钥',
      description: '使用生物识别或设备 PIN 码登录，无需密码。',
      register: '注册通行密钥',
      registering: '注册中...',
      success: '通行密钥注册成功',
      failed: '通行密钥注册失败',
      cancelled: '通行密钥注册已取消',
      alreadyRegistered: '此设备已注册为通行密钥。',
      deleted: '通行密钥已删除',
      deleteFailed: '删除通行密钥失败',
      renameFailed: '重命名通行密钥失败',
      noPasskeys: '尚未注册通行密钥。',
      unnamed: '未命名设备',
      registered: '注册时间',
      lastUsed: '上次使用',
    },
    login: {
      continueWithGoogle: '使用 Google 登录',
      continueWithMicrosoft: '使用 Microsoft 登录',
      continueWithApple: '使用 Apple 登录',
      or: '或',
      email: '电子邮件',
      phone: '手机',
      emailPlaceholder: 'you@company.com',
      phonePlaceholder: '+86 1XX XXXX XXXX',
      sendCode: '发送验证码',
      codeSentTo: '我们已向 {{destination}} 发送了6位验证码',
      verify: '验证',
      usePasskey: '使用通行密钥登录',
      signIn: '登录',
      authkitHint: '使用 SSO、邮箱或通行密钥登录',
      otherMethods: '其他登录方式',
      subtitle: '登录您的账户以继续',
      failedToSendCode: '发送验证码失败',
      invalidCode: '验证码无效',
      passkeyCancelled: '通行密钥验证已取消',
      passkeyFailed: '通行密钥验证失败',
      errors: {
        auth_failed: '认证失败，请重试。',
        missing_params: '登录过程中出现错误，请重试。',
        invalid_state: '登录会话已过期，请重试。',
        user_not_found: '未找到此邮箱对应的账户，请联系管理员。',
        unsupported_provider: '不支持此登录方式。',
        session_expired: '您的会话已过期，请重新登录。',
        unknown: '出现错误，请重试。',
      },
    },
  },

  tenant: {
    switchTenant: '切换租户',
    currentTenant: '当前租户',
  },

  language: {
    en: 'English',
    th: 'ไทย',
    zh: '中文',
    vi: 'Tiếng Việt',
    switchLanguage: '语言',
  },

  aiAgents: {
    title: 'AI智能体',
    subtitle: 'Papaya AI智能体驱动您的保险运营',
    agents: {
      claimAssessor: '理赔评估师',
      fraudDetector: '欺诈检测器',
      underwritingAssistant: '核保助手',
      documentAnalyzer: '文档分析器',
      complianceChecker: '合规检查器',
      lossPredictor: '赔付预测器',
    },
    statuses: {
      online: '在线',
      offline: '离线',
      processing: '处理中',
      idle: '空闲',
    },
    configure: '配置',
    viewLogs: '查看日志',
  },

  fatima: {
    name: 'Fatima',
    subtitle: '沙漠中的智慧女性',
    pageSubtitle: '您的AI保险运营助手',
    thinking: '思考中...',
    placeholder: '向 Fatima 提问...',
    pagePlaceholder:
      '向 Fatima 咨询保险运营相关问题...',
    disclaimer:
      'Fatima 可能会出错，请核实重要信息。',
    tryAsking: '试试问',
    askFatima: '问 Fatima',
    openFatima: '打开 Fatima',
    openInFatima: '在 Fatima 中打开',
    askWithQuery: '问 Fatima：\u201c{{query}}\u201d',
    welcome: `你好！我是 **Fatima** — 沙漠中的智慧女性。如同了解撒哈拉每一粒沙的风，我了解你数据中的每一笔理赔、每一份保单、每一个隐藏的规律。

我可以帮你：

- **理赔** — 查询状态、按患者或服务商查找理赔、解释审核决定
- **保单** — 搜索保单、查看保障详情、审查批注
- **核保** — 评估风险评分、检查申请状态
- **反欺诈** — 审查欺诈警报、调查可疑模式
- **服务商** — 查找服务商、检查合同状态
- **分析** — 赔付率、理赔趋势、KPI 汇总

你想了解什么？`,
    offlineFallback:
      '我目前连接出现问题。请确保服务正在运行。连接恢复后我就准备好了。',
    suggestions: {
      recentClaims: '显示最近的理赔',
      fraudAlerts: '有欺诈警报吗？',
      capabilities: '你能做什么？',
      lossRatio: '本月赔付率',
      fraudAlertsToday: '今天有欺诈警报吗？',
      whatIsLossRatio: '我们的赔付率是多少？',
      findPolicy: '查找Siam Group的保单',
      underwriting: '帮我核保',
    },
  },

  theme: {
    light: '浅色',
    dark: '深色',
    system: '跟随系统',
    switchTheme: '主题',
    customize: '自定义',
    customizeDesc: '选择颜色主题、调整圆角和选择显示模式。',
    color: '颜色',
    radius: '圆角',
    mode: '模式',
    reset: '恢复默认',
    colors: {
      papaya: 'Papaya',
      blue: '蓝色',
      green: '绿色',
      orange: '橙色',
      violet: '紫色',
      rose: '玫瑰',
      zinc: '灰色',
    },
  },

  commandPalette: {
    title: 'Oasis 命令面板',
    placeholder: '输入命令或向 Fatima 提问...',
    aiAssistant: 'AI助手',
    quickActions: '快捷操作',
    goTo: '前往',
    openHint: '打开',
    typeToAsk: '输入任何内容向 Fatima 提问',
    escToGoBack: '返回',
  },
  status: {
    title: '系统状态',
    subtitle: '所有 Oasis 服务的实时健康状况和运行时间',
    services: '服务',
    servicesUptime: '90天运行历史',
    recentIncidents: '近期事件',
    last7Days: '最近7天',
    overallUptime: '总体运行时间',
    activeIncidents: '进行中的事件',
    footer: '每60秒更新',
    daysAgo: '{{count}}天前',
    ninetyDaysAgo: '90天前',
    today: '今天',
    resolved: '已解决 {{time}}',
    statusLabels: {
      operational: '正常运行',
      degraded: '性能下降',
      outage: '严重故障',
      maintenance: '维护中',
    },
    severityLabels: {
      minor: '轻微',
      major: '严重',
      critical: '紧急',
    },
    overall: {
      operational: '所有系统正常运行',
      operationalDesc: '所有服务运行正常。',
      degraded: '部分系统性能下降',
      degradedDesc: '部分服务正在出现问题。',
      outage: '系统严重故障',
      outageDesc: '关键服务当前不可用。',
      maintenance: '计划维护',
      maintenanceDesc: '部分服务正在进行维护。',
    },
  },
} as const;

export default translations;
