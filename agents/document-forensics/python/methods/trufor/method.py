...
from .config import TruForArchConfig, TruForConfig, pretrained_arch, get_default_weights
...
class TruFor(BaseTorchMethod):
    ...
    def benchmark(self, image: Tensor, **kwargs) -> BenchmarkOutput:
        """
    ...
    @classmethod
    def from_config(cls, config: Optional[TruForConfig | dict | str | Path] = None):
        ...
        if "weights" not in config or config.get("weights") is None:
            logger.info("No weights provided, using default path `weights/trufor/`.")
            config["weights"] = get_default_weights()
