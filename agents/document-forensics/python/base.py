"""
Base classes for image forensics methods.

This module provides the base classes that all forensics methods should inherit from.
"""
...
class BaseTorchMethod(BaseMethod, Module):
    ...
    def load_weights(self, weights: Union[str, Path, dict]):
        ...
        if isinstance(weights, (str, Path)):
            # weights_only=False for PyTorch 2.6+ compatibility (default changed to True)
        ...
        elif "model" in weights_.keys():
            weights_ = weights_["model"]

        self.load_state_dict(
            weights_, assign=True
        )  # FIXME: assign limits torch version to >=2.1
